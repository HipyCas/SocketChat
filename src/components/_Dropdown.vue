<template>
	<div class="">
		<button class="z-10" @click="open = !open">
			<slot name="button" class="w-full"></slot>
		</button>
		...
		>
			<div
				class="md:hidden fixed inset-x-0 bottom-0 bg-white w-full z-40 px-2 py-2 shadow-2xl leading-loose"
				v-if="open"
			>
				<slot name="content"></slot>
			</div>
		...
		<div
			class="md:hidden fixed w-full h-full inset-0 bg-gray-900 opacity-50 z-20"
			@click="open = false"
			v-if="open"
		></div>
	</div>
